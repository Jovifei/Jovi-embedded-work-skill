...
**功能：** 为嵌入式 C 工程添加标准化注释，遵循 Jovi 代码规范。支持单文件和多文件并行处理。Ctrl+S 格式化与 `clangd_init` 共用 `.clang-format` 模板。
...
**功能：** 固定按 `ponytail → code_zl` 执行。先做最小的行为保持式简化，再使用 **code_zl V0.1.0** 整理嵌入式 C 的注释、分节和格式。两阶段不可交换，也不可只执行其中一个阶段。
...
→ 再用 code_zl V0.1.0 整理最终代码的函数头、行内注释和分节格式
→ 输出两个阶段的变更与验证结果
```
