...
- **V0.2.0**：新增“注释真实性 + 结构经济性”门禁：数字/单位/状态/硬件能力必须与代码一致；禁止把 ODR 写成物理触点、把 bool 1ms 标志写成绝对实时周期、把未上板路径写成已验证；发现安全宏靠传递 include、注释与实现 75%/80% 漂移、已删除 API/旧头复活时必须报告，不允许用漂亮注释掩盖。新增公共头/一行 wrapper 仍属于结构改动，转 `/code_wrt`。
...
`code_zl` 只整理注释与格式，但必须充当“文字真实性守门员”。如果源码事实和旧注释冲突，不能猜一个更顺眼的说法直接改上去；要指出冲突，并在可确认范围内按源码事实改注释，涉及行为取舍则交给 `code_wrt/code_sc`。

### 1. Comment-to-Code 一致性检查

整理范围内至少核对：

- 数值：750‰ 不能注释成 80%；
- 单位：mV/mA/mW/ms/‰ 不能省略或混写；
- 硬件：`ODR=1` 只能写“MCU 已命令继电器 ON”，不能写“触点已吸合”；
- 时序：单 bit 的 1ms flag 只能写“收到过至少一个 1ms 节拍”，不能宣称“每 1ms 必执行一次”；
- 并发：ISR 只置 pending 时，注释不能写成“ISR 已完成故障处理”；
- 验证：Host/Keil/静态检查通过不能写成“实板安全已验证”。

报告中遇到冲突必须列出位置、代码事实、旧注释、处理结论（已按事实修正 / 需要产品确认 / 需要上板确认）。

### 2. 安全相关注释禁止过度承诺

推荐真实写法：

- “PA13 ODR 已写高：表示 MCU 请求吸合，板上无触点反馈”；
- “首个非零 Duty 前打开 COMP2 live window；真实响应延迟需示波器确认”；
- “主循环消费 1ms 标志；若主循环超时，多个 tick 可能合并”；
- “预充 PV 下沿为经验防塌陷边界，不等同于 Vmpp 定位”。

### 3. 安全宏/条件编译可见性异常必须报警

如果看到 `#if (APP_xxx_ENABLE != 0U)`，但当前文件没有直接 include 该宏 Owner，或宏只靠多级传递 include 才可见，不能只补一句“默认开启保护”。应报告未定义按 0 的风险并交由 `/code_wrt` 修正 Owner/include/#error。`code_zl` 不自行重构 include 图，但不得用注释掩盖风险。

### 4. 文件少、头文件少是目标，但归属必须真实

`code_zl` 本身不新建 public header，也不为了分节把一个文件拆成多个文件。遇到一个头只有一个 DTO/几个宏时，可以建议交给 `/code_wrt` 合并到真实 Owner；但不能为了少文件把所有内容塞进 `main.h`。分节标题必须表达真实 Owner，例如“预充策略参数 / PWM 执行器时序 / 故障恢复窗口”，不要用模糊的“公共参数”。

### 5. 一行函数不因为加了长注释就变合理

如果函数体只有一行，且没有事务边界、集中副作用、复用、平台抽象或测试 seam，`code_zl` 不应给它写大段注释来掩盖过度封装。只报告“该 wrapper 可能可直接展开，结构是否合并交 `/code_wrt` 判断”。

### 6. todo 必须可验收

硬件/实时性/首拍保护 todo 必须写“测什么 + 怎么判通过”。例如：`// todo: 示波器确认首次非零 PWM 后 COMP2 下降沿到 MOE 关闭的延迟；判据：故障脉冲期间不得出现下一颗危险 PWM 脉冲。` 禁止只写“后面看看/待上板”。
