...
2. root README, root GUIDE, and explicitly maintained engineering records; process plans and Todo files are not durable memory
...
Map by meaning rather than numeric prefixes. Architecture and requirements feed overview; project relationships and cross-project learning feed relations; test reports and Git evidence feed progress; accepted tradeoffs feed decisions; repeatable verified commands and Bug theory feed workflow. Summarize the actual project docs in the workflow note, then mirror only safe Markdown originals to `05-工程文档/` with source-relative paths and hashes. Do not replace the summary with an external path or link, create a slot just to say “none”, or infer a percentage without evidence.
